...
contract Quiz {
   ...
   mapping(uint => QuizEvent) public quizzes;
   ...
   function makeQuiz (string memory _name, uint memory _pool, string memory _question, string memory _ans1, string memory _ans2, string memory _ans3, string memory _ans4 private {
      numQuizzes ++;
      quizzes[numQuizzes] = Quiz(numQuizzes, _name, _pool, Question(_question, _ans1, _ans2, _ans3, _ans4));
   }
   ...
   function scoreAttempt (uint _quizId, string _ans) returns (bool) {
      require(!quizzes[_quizId].attempts[msg.sender]);
      require(_quizId > 0 && _quizId < = numQuizzes);

      if(quizzes[_quizId].q1.correctAns == _ans) {
         return true;
      } else {
         return false;
      }
